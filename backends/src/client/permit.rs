...
impl PermissionsDal for Client {
    #[instrument(skip_all, fields(shuttle.user.id = user_id))]
    async fn get_user(&self, user_id: &str) -> Result<UserRead> {
    ...
    #[instrument(skip_all, fields(shuttle.user.id = user_id, shuttle.project.id = project_id))]
    async fn create_project(&self, user_id: &str, project_id: &str) -> Result<()> {
    ...
    #[instrument(skip_all, fields(shuttle.user.id = user_id, shuttle.team.id = team_id))]
    async fn delete_team(&self, user_id: &str, team_id: &str) -> Result<()> {
