...
impl ShuttleInit for ShuttleInitSerenity {
    ...
    fn get_dependency_attributes(&self) -> HashMap<&str, HashMap<&str, Value>> {
        ...
        (
            "serenity", 
            HashMap::from([
                ("features", Value::from(Array::from_iter(["client", "gateway", "rustls_backend", "model"]))),
...
impl ShuttleInit for ShuttleInitTower {
    ...
    fn get_dependency_attributes(&self) -> HashMap<&str, HashMap<&str, Value>> {
        ...
        (
            "tower", 
            HashMap::from([
                ("features", Value::from(Array::from_iter(["full"]))),
            ]),
        ),
        (
            "hyper", 
...
impl ShuttleInit for ShuttleInitThruster {
    ...
    fn get_dependency_attributes(&self) -> HashMap<&str, HashMap<&str, Value>> {
        ...
        (
            "thruster", 
            HashMap::from([
                ("features", Value::from(Array::from_iter(["hyper_server"]))),
            ]),
        ),
        ])
    }
