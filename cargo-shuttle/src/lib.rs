...
impl Shuttle {
    ...
    async fn deployment_redeploy(&self, deployment_id: String) -> Result<()> {
        ...
        self.track_deployment_status(pid, &deployment.id).await?;
    ...
    async fn deploy(&mut self, args: DeployArgs) -> Result<()> {
        ...
        if self.track_deployment_status(pid, &deployment.id).await? {
            for log in client.get_deployment_logs(pid, &deployment.id).await?.logs {
                if args.raw {
                    println!("{}", log.line);
                } else {
                    println!("{log}");
