...
async fn load(
    ...
    // TODO: get stripe price id from resource type?
    ...
    match response {
        Ok(response) => {
            ...
            if !cached_resources_contains_rds && loaded_resources_contains_rds {
                // TODO: this message should be generated by the resource type?
                deployment_updater.set_message(&deployment_id, "this deployment successfully provisioned an RDS instance, which will icnrease your subscription cost by $20").await.map_err(|err| {
