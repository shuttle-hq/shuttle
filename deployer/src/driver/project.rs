...
use crate::driver::task::TaskResult;
use crate::project::machine::Project;
use crate::project::name::ProjectName;
use crate::project::state::{EndState, Refresh, State};
use crate::DeployerService;

use super::task::{BoxedTask, Task};
...
// Worker queue size for the projects deployments
...
pub fn refresh() -> impl Task<ProjectContext, Output = Project, Error = Error> {
    run(|ctx: ProjectContext| async move {
        match ctx.state.refresh(&ctx.inner).await {
...
impl Task<ProjectContext> for RunUntilDone {
    ...
    async fn poll(&mut self, project_ctx: ProjectContext) -> TaskResult<Self::Output, Self::Error> {
        if !<Project as EndState<ContextProvider>>::is_done(&project_ctx.state) {
            TaskResult::Pending(project_ctx.state.next(&project_ctx.inner).await.unwrap())
        } else {
            TaskResult::Done(project_ctx.state)
        }
    }
}
    ...
    T: Task<ProjectContext, Output = Project, Error = Error>,
{
    ...
    async fn poll(&mut self, _: ()) -> TaskResult<Self::Output, Self::Error> {
        ...
        let project = match self
            .service
            .sqlite()
            .project_state(&self.project_name)
            .await
        {
            Ok(project) => project,
            Err(err) => {
                return TaskResult::Err(Error::custom(ErrorKind::ProjectNotFound, err.to_string()))
            }
        };

        let account_name = match self.service.sqlite().account(&self.project_name).await {
        ...
        let project_ctx = ProjectContext {
            ...
            inner: self.service.context().clone(),
            state: project,
        };
