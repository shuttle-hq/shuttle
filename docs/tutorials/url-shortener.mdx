...
I'm using [shuttle](https://www.shuttle.rs/) for this project. It's a serverless
...
[installed](https://docs.rs/shuttle-service/0.2.5/shuttle_service/#deploying)
and an [account](https://www.shuttle.rs/) so I simply:

```bash
mkdir -p ~/projects/url-shortener && cd ~/projects/url-shortener && cargo init --lib
```

Ok we have our cargo project. I stop and think for a little bit - which web
framework do I want to use? I think I'm going to go with Rocket (opens new
window). It's pretty much production ready with a sweet API and I'm reasonably
proficient with it.
...
My IDE violently lights up with red syntax highlighting as I realise I haven't
imported anything. The realities of software engineering hit me as I eye the
bottle of whiskey next to me. 18 year old scotch. It turns out I'm grossly
overpaid for the value I offer society. I grab a coffee mug and pour myself a
small shot - liquid courage.

Next I import all of the dependencies to get shuttle to work with Rocket -
pretty simple. I open up `Cargo.toml` add a couple of lines:
...
My IDE quietens down as dependencies are resolved and a wave of relief washes
over me. Let's deploy this thing.
...
[shuttle / sqlx](https://docs.rs/shuttle-service/0.2.5/shuttle_service/#using-sqlx)
...
unique id on the fly using the [nanoid](https://crates.io/crates/nanoid)crate
