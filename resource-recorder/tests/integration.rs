...
use shuttle_common_tests::docker::exec_psql;
use shuttle_common_tests::docker::DockerInstance;
...
const PG_CONTAINER_NAME: &str = "shuttle_resource_recorder_test_pg";
...
mod needs_docker {
    ...
    fn spawn_server(port: u16, db_name: String) -> JoinHandle<()> {
        ...
        tokio::task::spawn(async move {
            ...
            Server::builder()
                ...
                .add_service(ResourceRecorderServer::new(Service::new(pg)))
                .serve(addr)
                .await
                .unwrap()
        })
    ...
    async fn manage_resources() {
        ...
        let server_future = spawn_server(port, String::from("test_db"));
