...
impl ResourceTracker {
    ...
    pub fn get_cached_output(&self, r#type: Type, config: &Value) -> Option<Value> {
        ...
        // We always return the cached output for them, even if they change from a deployment where the
        // secrets where changed. We have to always call `output()` on them to get the latest secrets,
        // even if they updated, since we don't track a config for them that can change if secrets changed.
        if config.is_null() {
            return None;
        }
