...
description: DevLog[1] covers how we designed and built a deployment system
...
6. `Deleted`  - user-initiated deletion of the deployment
...
![Deployment State Machine](state-machine.png)

Even though we have a nice representation of our states - these states don't actually hold any data yet. We would like the `DeploymentState` to own all the data that corresponds to the specific stage in it's deployment. We'll create some structs to hold the data required for each stage.
...
When a deployment is queued, the shuttle build system writes the `crate_bytes` (just  a tarball of a cargo project) to the file system. It then extracts the tarball and starts the compilation process by using the `cargo` crate and running `cargo::ops::compile`.
...
So far so good. At this point we have a pointer to a compiled shared object file and we need to load it into memory.
 ...
 The reason we keep the `Library` struct around is that `Box<dyn Service>` is ultimately just a pointer to data loaded and managed by `Library`. Library going out of scope deallocates that data meaning service will be pointing to deallocated memory hence we get a `segfault`. For this reason we keep `Library` around for the lifetime of the deployment.
...
In the case of shuttle, using enum variants and structs to represent states in a state machine worked well. The states were distinct and clear, and for the most part the transitions are clean and self-contained.

So what do you think about enum variants as states in a state machine? What would have done differently?
