...
Before we go further we should note that async is not for process expensive operations. It's only beneficial for IO in which data comes from somewhere further away than the RAM and when there is a lot of it. For process expensive operations parallelization is beneficial.
...
Comparing a example written using async vs the same example written synchronously - for a large numbers of concurrent web requests, async Rust is 60% faster that synchronous requests and 20% faster than spinning up a thread for each request[^benchmarks].
...
#[tokio::main]
async fn main() {
    println!("Hello from a async function");
}
```
...
In a async function you can use `.await`. You add it on to the end of a call of async function and it will now block and return the actual value.

Async functions (and async blocks) return Futures. A [Future](https://doc.rust-lang.org/std/future/trait.Future.html) is a function which returns a [Poll](std::task::Poll). Poll is a bit like a `Result` or `Option`, it has two variants one is a final value and the other variant is that the value is still pending. Futures are lazy, there are two ways to run a future: `tokio::spawn` to spawn eagerly and get a [JoinHandle](https://docs.rs/tokio/latest/tokio/task/struct.JoinHandle.html) or `.await`. Rust warns against *unawaited* futures.
