...
To start the random we have to set up a seed. We will be doing something similar to the creation of the Tera instance. We must wrap it in an arc and a mutex because generating new identifiers requires mutable access. We now have the following main function:
...
Now we have the token we now need to make a query that gets user information. We can do that using SQL joins
